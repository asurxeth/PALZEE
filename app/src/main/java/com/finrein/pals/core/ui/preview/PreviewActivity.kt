package com.finrein.pals.core.ui.preview
...
import com.finrein.pals.feature.home.CapturedPreviewScreen
import com.finrein.pals.feature.home.HomeViewModel
import com.finrein.pals.feature.home.PalThemes
import com.finrein.pals.feature.home.handleVlogSubmission
...
class PreviewActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val activeVlogPal = if (activePalCode != null && activePalName != null) {
            com.finrein.pals.core.domain.model.PalItem(
                name = activePalName,
                code = activePalCode,
                size = "0",
        ...
        setContent {
            ...
            val vlogPrefs = remember { com.finrein.pals.feature.home.getVlogPrefs(context) }

            LaunchedEffect(Unit) {
                val saved = vlogPrefs.getString("created_pals", "") ?: ""
                val initialList = if (saved.isEmpty()) {
                    listOf(com.finrein.pals.core.domain.model.PalItem(name = "vlog", size = "12", code = "vlog", isVlog = true))
                } else {
            ...
            val allPalsSubmissions = remember {
                ...
                val initialMap = if (submissionsJson.isNotEmpty()) {
                    try {
                        kotlinx.serialization.json.Json.decodeFromString<Map<String, List<com.finrein.pals.core.domain.model.SubmissionDbItem>>>(submissionsJson)
                    } catch (e: Exception) {
                        emptyMap()
                    }
                } else {
